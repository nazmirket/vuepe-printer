module.exports = function compose(s) {
	...
	// transform
	const tParts = [
		`translate(${s.transform.translate.x || 0},${s.transform.translate.y || 0})`,
	...
	// background image
	if (s?.background?.image) parts.push(`background-image: url(${s.background.image})`)

	// background color
	if (s?.background?.color) parts.push(`background-color: ${s.background.color}`)
